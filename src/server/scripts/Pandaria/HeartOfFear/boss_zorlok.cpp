    ...
    public:
        ...
        struct boss_zorlokAI : public BossAI
        {
            ...
            void Reset() override
            {
                ...
                me->SetCanFly(true);
                me->RemoveUnitFlag(UnitFlags(UNIT_FLAG_IMMUNE_TO_PC | UNIT_FLAG_PACIFIED));
                // me->SetAnimationTier(UnitAnimationTier::Hover);
                me->SetDisableGravity(true);
                me->SetReactState(REACT_AGGRESSIVE);
            ...
            void SetFlying(uint32 point, bool last = false)
            {
                ...
                me->AddUnitFlag(UnitFlags(UNIT_FLAG_IMMUNE_TO_PC | UNIT_FLAG_PACIFIED));
    ...
    void SelectTargets(std::list<WorldObject*>& targets)
    {
        if (Unit* caster = GetCaster())
        {
            ...
            if (pList.size() >= maxVictims)
            {
                ...
                if (pList.size() > maxVictims)
                    Trinity::Containers::RandomResize(targets, maxVictims);
...
void AddSC_boss_zorlok()
{
    ...
    RegisterSpellScript(spell_convert); // 122740 - Convert
    RegisterSpellScript(spell_zorlok_achievement_loot_bug); /*
