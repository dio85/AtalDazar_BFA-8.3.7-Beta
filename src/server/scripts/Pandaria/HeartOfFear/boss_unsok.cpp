    ...
    public:
        ...
        struct boss_amber_shaper_unsokAI : public BossAI
        {
            ...
            void UpdateAI(uint32 diff) override
            {
                ...
                CheckHomeDistToEvade(diff, 175.5f);
            }
            ...
            private:
                ...
                struct ReshapeLifeSecondTankPredicate
                {
                    public:
                        ...
                        {
                            return object && object->ToPlayer() && (object->ToPlayer()->GetRoleForGroup() != ROLE_TANK || (_me->GetVictim() && object->GetGUID() == _me->GetVictim()->GetGUID()));
                        }
                ...
                if (Unit* caster = GetCaster())
                {
                    if (Unit* target = GetHitUnit())
                    {
                        if (SpellInfo const* sInfo = GetSpellInfo())
                        {
                            ...
                            {
                                if ((int32)wPower - sInfo->GetEffect(EFFECT_0)->BasePoints <= 0)
                                    caster->Kill(target);
                                else
                                    target->SetPower(POWER_ALTERNATE_POWER, wPower - sInfo->GetEffect(EFFECT_0)->BasePoints);
                            }
                            else
                                caster->Kill(target);
                        }
...
void AddSC_boss_unsok()
{
    ...
    RegisterSpellScript(spell_amber_strike);
    new spell_consume_amber();
    RegisterSpellScript(spell_break_free);
    RegisterSpellScript(spell_fling_dummy);
    RegisterSpellScript(spell_fling);
    RegisterSpellScript(spell_fling_jump);
    new spell_burst_living_amber();
    new spell_unsok_reshape_of_life();
    RegisterAuraScript(spell_unsok_reshape_of_life);
    new spell_unsok_parasitic_growth();
    new spell_amber_globule_detonation();
    new spell_reshape_life();
    new spell_destroy_will();
    RegisterAuraScript(spell_unsok_burning_amber_triggering);
