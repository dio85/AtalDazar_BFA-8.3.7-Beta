...
class npc_wrathion : public CreatureScript
{
    public:
        ...
        bool OnQuestAccept(Player* player, Creature* creature, Quest const* quest) override
        {
            ...
            switch (quest->GetQuestId())
            {
                ...
                case QUEST_TEST_OF_VALOR_A:
                    if (player->HasAchieved(quest->GetQuestId() == QUEST_TEST_OF_VALOR_A ? 8030 : 8031))
        ...
        bool OnGossipHello(Player* player, Creature* creature) override
        {
            ...
            if (player->GetQuestStatus(QUEST_LEGEND_IN_THE_MAKING) == QUEST_STATUS_INCOMPLETE)
                AddGossipItemFor(player, GOSSIP_ICON_CHAT, WRATHION_GOSSIP, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
            /*
            if (player->GetQuestStatus(QUEST_MEASURE_OF_THE_LEADER) == QUEST_STATUS_INCOMPLETE)
                AddGossipItemFor(player, GOSSIP_ICON_CHAT, WRATHION_GOSSIP, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 2);*/
        ...
        struct npc_tong_the_fixer_legend_in_the_makingAI : public ScriptedAI
        {
            ...
            void IsSummonedBy(Unit* summoner) override
            {
                ...
                if (summoner && summoner->ToPlayer())
                {
                    ...
                    {
                        AddTimedDelayedOperation(53000, [this]() -> void
                        {
                            me->HandleEmoteCommand(EMOTE_ONESHOT_BOW);
                        });
...
struct npc_wrathion_niuzao_celestial : public ScriptedAI
{
    ...
    void IsSummonedBy(Unit* summoner) override
    {
            ...
            .Schedule(Milliseconds(3500), [this](TaskContext context)
        {
            ...
            delay = 0;// me->GetSplineDuration() + 2500;
            scheduler
                .Schedule(Milliseconds(delay), [this](TaskContext context)
            {
                Talk(TALK_SPECIAL_1);
...
struct npc_wrathion_xuen_challenge : public ScriptedAI
{
    npc_wrathion_xuen_challenge(Creature* creature) : ScriptedAI(creature) { }
...
void AddSC_the_veiled_stair()
{
    ...
    RegisterCreatureAI(npc_darkhatched_soulbleeder);
    RegisterCreatureAI(npc_leechfingers);
    RegisterCreatureAI(npc_hatescale_spitter);
    RegisterCreatureAI(npc_hatescale_ironface);
    RegisterCreatureAI(npc_image_of_wrathion);
    RegisterCreatureAI(npc_wrathion_suen_celestial);
    RegisterCreatureAI(npc_wrathion_chiji_celestial);
    RegisterCreatureAI(npc_wrathion_niuzao_celestial);
    RegisterCreatureAI(npc_wrathion_xuen_challenge);
    RegisterCreatureAI(npc_neltharions_tear);
    RegisterCreatureAI(npc_xuen_challenge_gong);
    RegisterCreatureAI(npc_wrathion_cloak_achieve);
    RegisterCreatureAI(npc_wrathion_celestial_spirits_cloak);
    RegisterCreatureAI(npc_willy_wilder);

    RegisterSpellScript(spell_wrathion_holding_a_cup);
    RegisterSpellScript(spell_wrathion_blindside);
    RegisterSpellScript(spell_legendary_wrathion_essence);
    RegisterAuraScript(spell_summon_wrathion_celestials);
    RegisterAuraScript(spell_wrathion_reign_of_fire);
    RegisterAuraScript(spell_wrathion_legendary_finale);
    RegisterAuraScript(spell_violets_and_blueberries);
