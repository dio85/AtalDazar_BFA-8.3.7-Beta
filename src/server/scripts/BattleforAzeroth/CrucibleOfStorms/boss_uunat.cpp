		...
		void UpdateAI(uint32 diff)
		{
			...
			while (uint32 eventId = events.ExecuteEvent())
			{
				...
				case EVENT_FIXATE_TANK:
				{
					...
					for (auto player : playerList)
					{
						me->GetThreatManager().AddThreat(player, 9999999.9f);
...
public:
	...
	class bfa_spell_unstable_resonance_remove_effect_AuraScript : public AuraScript
	{
		...
		void Register()
		{
			OnEffectRemove += AuraEffectRemoveFn(bfa_spell_unstable_resonance_remove_effect_AuraScript::OnRemove, EFFECT_0, SPELL_AURA_PERIODIC_DAMAGE, AURA_EFFECT_HANDLE_REAL);
		}
	};
